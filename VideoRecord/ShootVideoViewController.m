...
-(void)initCapture{
    ...
    imageView.image = [UIImage imageNamed:@"viewBG.png"];
    

    
    //背景 倒计时
     _circle = [[XLCircleProgress alloc]initWithFrame:CGRectMake(0, 0, 76, 76)];
    _circle.center = CGPointMake(SCREEN_WIDTH/2, 80+64+preLayerHeight+20+43);
    [self.view addSubview:_circle];
    
    //开始
    shootBt = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 66, 66)];
    shootBt.center = CGPointMake(38, 38);
//    shootBt.backgroundColor = UIColorFromRGB(0xfa5f66);
    shootBt.backgroundColor = [UIColor whiteColor];
    [shootBt setTitle:@"开始" forState:UIControlStateNormal];
    [shootBt setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [shootBt addTarget:self action:@selector(shootButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [shootBt makeCornerRadius:33 borderColor:UIColorFromRGB(0x28292b) borderWidth:3];
    [_circle addSubview:shootBt];
    
    //完成
    finishBt = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 50, 50)];
    finishBt.center = CGPointMake(SCREEN_WIDTH-35,80+64+preLayerHeight+20+45);
...
//    [finishBt setBackgroundImage:[UIImage imageNamed:@"shootFinish"] forState:UIControlStateNormal];
    ...
    //重拍
    remakeBt = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 50, 50)];
    remakeBt.center = CGPointMake(35,80+64+preLayerHeight+20+45);
    remakeBt.adjustsImageWhenHighlighted = NO;
//    [remakeBt setBackgroundImage:[UIImage imageNamed:@"shootFinish"] forState:UIControlStateNormal];
    [remakeBt setTitle:@"重拍" forState:UIControlStateNormal];
    [remakeBt addTarget:self action:@selector(go) forControlEvents:UIControlEventTouchUpInside];
    remakeBt.hidden = YES;
...
//    [AVCaptureDeviceInput deviceInputWithDevice:[self getCameraDeviceWithPosition:AVCaptureDevicePositionBack] error:nil];
...
//    [shootBt setTitle:@"" forState:UIControlStateNormal];
...
- (void)onTimer:(NSTimer *)timer
{
    ...
    //圆
    _circle.progress = currentTime/30.0f;
    if (currentTime >= 30.0) {
        [shootBt setTitle:[NSString stringWithFormat:@"0"] forState:UIControlStateNormal];
    }else{
        [shootBt setTitle:[NSString stringWithFormat:@"%.0f",30.0-currentTime] forState:UIControlStateNormal];
    }
...
//        [_captureVideoPreviewLayer addAnimation:animation forKey:nil];
//        if (newInput != nil) {
